...
if ($action == 'ajaxcheckout') {
   ...
   foreach ($userCartData as $item) {
       ...
       $watch_title = $item['title'];
       $watch_price = $item['price'];
       $quantity = $item['quantity'];
       $productimage = $item['productImage'];

        // Thực hiện truy vấn cập nhật thông tin WATCH
        $sql = "INSERT INTO User_cart ( user_id, watch_id, watch_title, watch_price, quantity, productimage, order_total, created_at, updated_at)
        VALUES ('1','$watch_id', '$watch_title', '$watch_price', '$quantity', '$productimage', '$order_total', NOW(), NOW())";
